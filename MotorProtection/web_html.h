...
th,td{padding:10px 12px;text-align:left;border-bottom:1px solid var(--border);font-variant-numeric:tabular-nums}
...
form{display:grid;grid-template-columns:220px 1fr;gap:12px 16px;max-width:720px;background:var(--surface);padding:20px;border:1px solid var(--border);border-radius:var(--r)}
...
.bar{height:8px;background:#0f172a;border-radius:99px;overflow:hidden;min-width:80px}
...
<main>
  ...
  <section class="page on" id="p-dash">
    ...
    <table id="dash-tbl" hidden>
      <thead><tr>
        ...
        <th>RMS (A)</th><th>Thermal</th><th></th>
      </tr></thead>
      <tbody id="dash-body"></tbody>
    </table>
    <p style="margin-top:16px"><button id="btn-cal">Calibrate zero-current</button></p>
  </section>
  <section class="page" id="p-add">
    ...
    <form id="f-add">
      ...
      <label for="a-hz">Mains frequency</label>
      <select id="a-hz" name="hz"><option value="50">50 Hz</option><option value="60">60 Hz</option></select>
  ...
  <section class="page" id="p-edit">
    ...
    <form id="f-edit" hidden>
      ...
      <label for="e-hz">Mains frequency</label>
      <select id="e-hz"><option value="50">50 Hz</option><option value="60">60 Hz</option></select>
  ...
  <section class="page" id="p-log">
    ...
    <div id="log-wrap">
      <table>
        <thead><tr><th>Uptime (ms)</th><th>Motor</th><th>Type</th><th>Current (A)</th></tr></thead>
...
let DATA={motors:[],sd_ok:0,heap:0,calibrated:0,free_ch:0};
...
function renderDash(){
  $("meta").textContent="heap "+DATA.heap+"  |  SD "+(DATA.sd_ok?"ok":"none")+"  |  free ch "+DATA.free_ch+"  |  cal "+(DATA.calibrated?"yes":"no");
  ...
  used.forEach(m=>{
    ...
    const canStart=m.status===0;
    const canStop=m.status===1;
    const canReset=m.status===2||m.status===3;
    const tr=document.createElement("tr");
    tr.innerHTML="<td>"+esc((m.channels||[]).join(", "))+"</td><td>"+esc(m.name)+"</td>"+
      "<td><span class='badge s-"+st+"'>"+st+"</span></td>"+
      "<td>"+fmtU(m.uptime_ms)+"</td><td>"+m.fault_count+"</td>"+
      "<td>"+rmsCell(m)+"</td><td>"+thBar(m.thermal_pct)+"</td>"+
      "<td class='row-actions'>"+
      "<button class='btn-ok' data-a='start' data-i='"+m.idx+"' "+(canStart?"":"disabled")+">Start</button>"+
...
$("dash-body").addEventListener("click",ev=>{
  ...
  post("/api/"+a,"idx="+i).then(()=>poll());
});
$("btn-cal").onclick=()=>post("/api/calibrate","").then(r=>flash(r.ok?"Calibrated.": (r.err||"calibrate failed"), r.ok?"okmsg":"err"));
...
$("a-ac").onchange=()=>{$("a-hz").disabled=$("a-ac").value==="0"};
$("e-ac").onchange=()=>{$("e-hz").disabled=$("e-ac").value==="0"};
function fd(form){return new URLSearchParams(new FormData(form)).toString()}
$("f-add").onsubmit=ev=>{
  ev.preventDefault();
  const n=+$("a-n").value;
  let body="phases="+$("a-ph").value+"&name="+encodeURIComponent($("a-name").value)+
    "&in="+$("a-in").value+"&ac="+$("a-ac").value+"&hz="+$("a-hz").value+
...
$("f-edit").onsubmit=ev=>{
  ...
  let body="idx="+i+"&name="+encodeURIComponent($("e-name").value)+
    "&in="+$("e-in").value+"&ac="+$("e-ac").value+"&hz="+$("e-hz").value+
...
function loadLog(){
  fetch("/api/log",{credentials:"same-origin"}).then(guard).then(r=>r.json()).then(j=>{
    ...
    (j.rows||[]).forEach(row=>{
      const tr=document.createElement("tr");
      tr.innerHTML="<td>"+esc(row.uptime_ms)+"</td><td>"+esc(row.motor)+"</td><td>"+esc(row.type)+"</td><td>"+esc(row.current_A)+"</td>";
      tb.appendChild(tr);
    });
    if(!(j.rows||[]).length && j.sd_ok){
      tb.innerHTML="<tr><td colspan='4' class='empty'>No fault entries.</td></tr>";
    }
  });
}
$("log-clear").onclick=()=>{
...
function poll(){
  fetch("/api/status",{credentials:"same-origin"}).then(guard).then(r=>r.json()).then(j=>{DATA=j;renderDash();if($("p-edit").classList.contains("on"))fillEditPick();})
    .catch(()=>{});
}
...
.card{width:420px;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:32px}
